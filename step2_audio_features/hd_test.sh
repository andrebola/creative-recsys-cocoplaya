...
audio_path="/datasets/MTG/audio/stable/spotify-recs/audio/"
ls $audio_path |sort -R |while read first_dir; do
  ls "$audio_path/$first_dir" | while read entry ; do
    ...
    output_folder_ess="/datasets/MTG/projects/spotify-recs/essentia/${str_fn}/${str_sn}"
    if [ ! -f "${output_folder_ess}/${base}.json" ]; then
      mkdir -p ${output_folder_ess}
      echo "Reading $audio_path/$first_dir/$entry"
      ./streaming_extractor_music_b2 "$audio_path/$first_dir/$entry" "$output_folder_ess/${base}.json" profile.yaml || true
    fi
  done
done
